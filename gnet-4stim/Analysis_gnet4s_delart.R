...
ANOVA_gnet4s_delart_ST <- aov_ez(id = "multichar_knowresp",dv = "resp_right",within = "type",data = filter(gnet4s_delart_StudyRespSummary, !(multichar_knowresp %in% exclude_gnet4s_delart) & type != "prototype"))
print(ANOVA_gnet4s_delart_ST)
PostHoc_gnet4s_delart_ST <- emmeans(ANOVA_gnet4s_delart_ST, ~ type)
pairs(PostHoc_gnet4s_delart_ST)
contrast(PostHoc_gnet4s_delart_ST, "poly")

library(ggplot2)
png('gnet4s_delart_STUDY.png')
ggplot(filter(gnet4s_delart_StudyRespSummary, !(multichar_knowresp %in% exclude_gnet4s_delart) & type != "prototype"), aes(x=type, y=resp_right, color = type)) + 
...
a <- c(select(filter(gnet4s_delart_TestRespSummary, !(multichar_knowresp %in% exclude_gnet4s_delart) & tq == "old"),resp_left))$resp_left
b <- c(select(filter(gnet4s_delart_TestRespSummary, !(multichar_knowresp %in% exclude_gnet4s_delart) & tq == "new"),resp_left))$resp_left

diff <- a - b

shapiro.test(diff)

TTEST_gnet4s_delart_T <- t.test(a,b,paired = TRUE)

COHEND_gnet4s_delart_T <- cohen.d(a,b,paired = TRUE,conf.level=0.95)

WILCOXT_gnet4s_delart_T <- wilcox.test(a,b,paired = TRUE,conf.level = 0.95)

rm(a,b)

png('gnet4s_delart_TEST1.png')
ggplot(filter(gnet4s_delart_TestRespSummary,!(multichar_knowresp %in% exclude_gnet4s_delart)), aes(tq,resp_left, color=type)) +
...
gnet4s_delart_TestRespByType <- left_join(filter(gnet4s_delart_TestRespSummary,!(multichar_knowresp %in% exclude_gnet4s_delart) & tq=="old"),
                                         filter(gnet4s_delart_TestRespSummary,!(multichar_knowresp %in% exclude_gnet4s_delart) & tq=="new"),
                                         by=c("multichar_knowresp","type"))

colnames(gnet4s_delart_TestRespByType) <- c("multichar_knowresp","type","tq.x","noresp_O","maybe_O","recog_O","miss_O",
                                           "tq.y","noresp_N","maybe_N","fa_N","reject_N")

gnet4s_delart_TestRespByType <- select(gnet4s_delart_TestRespByType,-tq.x,-tq.y)

gnet4s_delart_TestRespByType <- mutate(gnet4s_delart_TestRespByType,CorrRecog = recog_O - fa_N)
gnet4s_delart_TestRespByType <- mutate(gnet4s_delart_TestRespByType,CorrRejec = reject_N - miss_O)
gnet4s_delart_TestRespByType <- mutate(gnet4s_delart_TestRespByType,OverallCCat = CorrRecog + CorrRejec)
