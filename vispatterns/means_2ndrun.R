...
Study_ResponsesByType <-
  GNET_pilot_filtered[is.na(count_test_sequence),.N,by=.(response,workerId,type)]
# we need to add some zeros for the type*tq*rkey(response) combinations that are left out

mycols  = c("response","workerId","type")
...
Test_CorrectResponsesByType <-
  GNET_pilot_filtered[!is.na(count_test_sequence), .N,by=.(response,workerId,tq,type)]
# we need to add some zeros for the type*tq*rkey(response) combinations that are left out

mycols  = c("response","workerId","tq","type")
...
Test_NewPResponses <- Test_NewPResponses[,c("workerId","type",
                                            "maybe_N","noresp_N","fa_N","reject_N")]
rm(o1,o2,o3,o4,Test_CorrectResponsesByType)

Test_ResponsesByType <- merge(Test_OldPResponses,Test_NewPResponses,by=c("workerId","type"))
