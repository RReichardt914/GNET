...
for (i in 1:10){
  ...
  for (j in 0:((nrow(cpdat)/3)-1)){
    ...
    if (is.na(ad)){ad <- 0}
    dsum <- dsum + ad
    difftoprot <- rbind(difftoprot, list(compname, dsum, cd, sd, ad))
  }
}

# we have to calculate the diffarr pictures differently
difftoprotda <- data.table(pict = character(),
                           dsum = numeric(),
                           cd = numeric(),
                           sd = numeric(),
                           ad = numeric())
for (i in 1:5){
  ...
  for (j in 0:((nrow(cpdat)/3)-1)){
    ...
    if (is.na(ad)){ad <- 0}
    dsum <- dsum + ad
    difftoprotda <- rbind(difftoprotda, list(compname, dsum, cd, sd, ad))
  }
}
...
for (j in 0:((nrow(picdat)/3)-1)){
  ...
  difftoprots <- rbind(difftoprots, list(compname, mean(diffs), sum(diffs),
                                         mean(cdifs), sum(cdifs),
                                         mean(sdifs), sum(sdifs),
                                         mean(adifs), sum(adifs)))
}
...
for (i in 1:length(novcats)){
  ...
  for (j in 0:((nrow(eval(parse(text=paste(whichcat,sep=""))))/3)-1)){
    ...
    eval(parse(text=paste(compname,"col <- c()",sep="")))
    eval(parse(text=paste(compname,"sha <- c()",sep="")))
    eval(parse(text=paste(compname,"arr <- c()",sep="")))
    ...
    for (k in 0:((nrow(eval(parse(text=paste(whichcat,sep=""))))/3)-1)){
      ...
      if (compname != cname){
        ...
        eval(parse(text=paste(compname,"col <- c(",compname,"col,cd)",sep="")))
        eval(parse(text=paste(compname,"sha <- c(",compname,"sha,sd)",sep="")))
        eval(parse(text=paste(compname,"arr <- c(",compname,"arr,ad)",sep="")))
      }
    }
    eval(parse(text=paste("diffinnovs <- rbind(diffinnovs,list(compname,mean(",compname,"),sum(",compname,
                                                                     "),mean(",compname,"col),sum(",compname,
                                                                     "col),mean(",compname,"sha),sum(",compname,
                                                                     "sha),mean(",compname,"arr),sum(",compname,
                                                                     "arr)))",sep="")))
  }
}
...
for (i in 1:length(novcats)){
  ...
  for (j in 0:((nrow(eval(parse(text=paste(whichcat,sep=""))))/3)-1)){
    ...
    eval(parse(text=paste("diffinnovs2 <- rbind(diffinnovs2,list(compname,mean(",compname,"),sum(",compname,
                          "),mean(",compname,"col),sum(",compname,
                          "col),mean(",compname,"sha),sum(",compname,
                          "sha),mean(",compname,"arr),sum(",compname,
                          "arr)))",sep="")))
...
colnames(difftoprot) <- c("pict","DTF","ColDTF","ShaDTF","ArrDTF","type")
colnames(difftoprots) <- c("pict","DTAF","dss",
                           "ColDTAF","cdss",
                           "ShaDTAF","sdss",
                           "ArrDTAF","adss","type")
difftoprots$dss <- NULL
difftoprots$cdss <- NULL
difftoprots$sdss <- NULL
difftoprots$adss <- NULL

difference_indices <- merge(difftoprot, difftoprots, by=c("pict","type"))
colnames(diffinnovs) <- c("pict","DTAN","dalls",
                          "ColDTAN","dcs",
                          "ShaDTAN","dss",
                          "ArrDTAN","das","type" )
...
colnames(diffinnovs2) <- c("pict","DTN_byPhase","dalls",
                           "ColDTN_bP","dcs",
                           "ShaDTN_bP","dss",
                           "ArrDTN_bP","das","type","phase")
diffinnovs2$dalls <- NULL
diffinnovs2$dcs <- NULL
diffinnovs2$dss <- NULL
diffinnovs2$das <- NULL
