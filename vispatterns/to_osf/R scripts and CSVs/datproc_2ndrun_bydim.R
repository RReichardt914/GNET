...
GNET_pilot_RUN2_alldata$ColDTF <- 0
GNET_pilot_RUN2_alldata$ShaDTF <- 0
GNET_pilot_RUN2_alldata$ArrDTF <- 0

GNET_pilot_RUN2_alldata$DTAF <- 0
GNET_pilot_RUN2_alldata$ColDTAF <- 0
GNET_pilot_RUN2_alldata$ShaDTAF <- 0
GNET_pilot_RUN2_alldata$ArrDTAF <- 0

GNET_pilot_RUN2_alldata$DTAN <- 0
GNET_pilot_RUN2_alldata$ColDTAN <- 0
GNET_pilot_RUN2_alldata$ShaDTAN <- 0
GNET_pilot_RUN2_alldata$ArrDTAN <- 0

GNET_pilot_RUN2_alldata$DTN_byPhase <- 0
GNET_pilot_RUN2_alldata$ColDTN_bP <- 0
GNET_pilot_RUN2_alldata$ShaDTN_bP <- 0
GNET_pilot_RUN2_alldata$ArrDTN_bP <- 0
for (i in 1:nrow(GNET_pilot_RUN2_alldata)){
  for (j in 1:nrow(difference_indices)){
    if (grepl(difference_indices$pict[j],GNET_pilot_RUN2_alldata$stim[i])){
      ...
      GNET_pilot_RUN2_alldata$ColDTF[i] <- difference_indices$ColDTF[j]
      GNET_pilot_RUN2_alldata$ShaDTF[i] <- difference_indices$ShaDTF[j]
      GNET_pilot_RUN2_alldata$ArrDTF[i] <- difference_indices$ArrDTF[j]
      
      GNET_pilot_RUN2_alldata$DTAF[i] <- difference_indices$DTAF[j]
      GNET_pilot_RUN2_alldata$ColDTAF[i] <- difference_indices$ColDTAF[j]
      GNET_pilot_RUN2_alldata$ShaDTAF[i] <- difference_indices$ShaDTAF[j]
      GNET_pilot_RUN2_alldata$ArrDTAF[i] <- difference_indices$ArrDTAF[j]
      
      GNET_pilot_RUN2_alldata$DTAN[i] <- difference_indices$DTAN[j]
      GNET_pilot_RUN2_alldata$ColDTAN[i] <- difference_indices$ColDTAN[j]
      GNET_pilot_RUN2_alldata$ShaDTAN[i] <- difference_indices$ShaDTAN[j]
      GNET_pilot_RUN2_alldata$ArrDTAN[i] <- difference_indices$ArrDTAN[j]
      
      GNET_pilot_RUN2_alldata$DTN_byPhase[i] <- difference_indices$DTN_byPhase[j]
      GNET_pilot_RUN2_alldata$ColDTN_bP[i] <- difference_indices$ColDTN_bP[j]
      GNET_pilot_RUN2_alldata$ShaDTN_bP[i] <- difference_indices$ShaDTN_bP[j]
      GNET_pilot_RUN2_alldata$ArrDTN_bP[i] <- difference_indices$ArrDTN_bP[j]
